...
- ✅ **Registrierung & Passwort-Reset (🔴 → ✅):** Selbst-Registrierung neuer Kunden mit Validierung (Name, E-Mail-Format, Passwortstärke, Eindeutigkeit) und Passwort-Reset per Token; neutrale Antwort gegen Konto-Enumeration; App personalisiert auf den angemeldeten Nutzer. Der E-Mail-Versand des Links ist ehrlich gestubbt (geloggt; Dev-Token nur mit `MAESTRO_DEV=1`).
- ✅ **DSGVO-Selbstbedienung (🔴 → teilweise ✅):** Einwilligung bei Registrierung (Pflicht + Zeitstempel), echter **Datenexport** (JSON-Download, Art. 20) und echte **Kontolöschung** inkl. aller zugehörigen Daten (Art. 17). Die zuvor gefakte Datenschutz-UI ist verdrahtet. Per API- (23 Tests) und Browser-Tests abgesichert. **Weiterhin offen:** Verschlüsselung ruhender Daten, Rechtstexte (Datenschutzerklärung/Impressum/AVV), Löschfristen.

Die Kategorientabellen unten zeigen weiterhin den **ursprünglichen** Prüfstand; die obigen Punkte sind damit teilweise bereits abgehakt. **Weiterhin offen** bleiben v. a. echte Datenbank, restliche DSGVO-Pflichten (Verschlüsselung, Rechtstexte), Zahlungen, echte Video-Calls, E-Mail-/Push-Versand, native Apps.
